...
# define I2C_TRANSVER(ns, i, id, sendData, sendLen, recvData, recvLen, waittime) int32_t i2c_transver(struct i2c *i, uint16_t id, uint8_t *sendData, uint32_t sendLen, uint8_t *recvData, uint32_t recvLen, TickType_t waittime)
...
# define I2C_TRANSVER_ISR(ns, struct i2c *i, uint16_t id, uint8_t *sendData, uint32_t sendLen, uint8_t *recvData, uint32_t recvLen) int32_t i2c_transverISR(struct i2c *i, uint16_t id, uint8_t *sendData, uint32_t sendLen, uint8_t *recvData, uint32_t recvLen);
#else
# define I2C_OPS(ns) static const struct i2c_ops ns##_i2c_ops = { \
	...
	.i2c_transver = &ns##_i2c_transver, \
	.i2c_sendISR = &ns##_i2c_sendISR, \
	.i2c_recvISR = &ns##_i2c_recvISR, \
	.i2c_transverISR = &ns##_i2c_transverISR, \
}
...
# define I2C_TRANSVER_ISR(ns, i, id, sendData, sendLen, recvData, recvLen) int32_t ns##_i2c_transverISR(struct i2c *i, uint16_t id, uint8_t *sendData, uint32_t sendLen, uint8_t *recvData, uint32_t recvLen)
#endif
/**\}*/
#endif
